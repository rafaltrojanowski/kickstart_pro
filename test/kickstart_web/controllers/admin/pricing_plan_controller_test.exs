defmodule KickstartWeb.Admin.PricingPlanControllerTest do
  ...
  describe "index" do
    test "lists all pricing_plans", %{conn: conn} do
      conn = get conn, Routes.admin_pricing_plan_path(conn, :index)
  ...
  describe "new pricing_plan" do
    test "renders form", %{conn: conn} do
      conn = get conn, Routes.admin_pricing_plan_path(conn, :new)
  ...
  describe "create pricing_plan" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, Routes.admin_pricing_plan_path(conn, :create), pricing_plan: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.admin_pricing_plan_path(conn, :show, id)

      conn = get conn, Routes.admin_pricing_plan_path(conn, :show, id)
      assert html_response(conn, 200) =~ "Pricing plan Details"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, Routes.admin_pricing_plan_path(conn, :create), pricing_plan: @invalid_attrs
      assert html_response(conn, 200) =~ "New Pricing plan"
    end
  end
  ...
  describe "edit pricing_plan" do
    ...
    test "renders form for editing chosen pricing_plan", %{conn: conn, pricing_plan: pricing_plan} do
      conn = get conn, Routes.admin_pricing_plan_path(conn, :edit, pricing_plan)
  ...
  describe "update pricing_plan" do
    ...
    test "redirects when data is valid", %{conn: conn, pricing_plan: pricing_plan} do
      conn = put conn, Routes.admin_pricing_plan_path(conn, :update, pricing_plan), pricing_plan: @update_attrs
      assert redirected_to(conn) == Routes.admin_pricing_plan_path(conn, :show, pricing_plan)

      conn = get conn, Routes.admin_pricing_plan_path(conn, :show, pricing_plan)
      assert html_response(conn, 200) =~ "some updated description"
    end

    test "renders errors when data is invalid", %{conn: conn, pricing_plan: pricing_plan} do
      conn = put conn, Routes.admin_pricing_plan_path(conn, :update, pricing_plan), pricing_plan: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Pricing plan"
    end
  end
  ...
  describe "delete pricing_plan" do
    ...
    test "deletes chosen pricing_plan", %{conn: conn, pricing_plan: pricing_plan} do
      conn = delete conn, Routes.admin_pricing_plan_path(conn, :delete, pricing_plan)
