defmodule KickstartWeb.Admin.FaqControllerTest do
  ...
  describe "index" do
    test "lists all faqs", %{conn: conn} do
      conn = get conn, Routes.admin_faq_path(conn, :index)
  ...
  describe "new faq" do
    test "renders form", %{conn: conn} do
      conn = get conn, Routes.admin_faq_path(conn, :new)
  ...
  describe "create faq" do
    test "redirects to show when data is valid", %{conn: conn} do
      conn = post conn, Routes.admin_faq_path(conn, :create), faq: @create_attrs

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.admin_faq_path(conn, :show, id)

      conn = get conn, Routes.admin_faq_path(conn, :show, id)
      assert html_response(conn, 200) =~ "Faq Details"
    end

    test "renders errors when data is invalid", %{conn: conn} do
      conn = post conn, Routes.admin_faq_path(conn, :create), faq: @invalid_attrs
      assert html_response(conn, 200) =~ "New Faq"
    end
  end
  ...
  describe "edit faq" do
    ...
    test "renders form for editing chosen faq", %{conn: conn, faq: faq} do
      conn = get conn, Routes.admin_faq_path(conn, :edit, faq)
  ...
  describe "update faq" do
    ...
    test "redirects when data is valid", %{conn: conn, faq: faq} do
      conn = put conn, Routes.admin_faq_path(conn, :update, faq), faq: @update_attrs
      assert redirected_to(conn) == Routes.admin_faq_path(conn, :show, faq)

      conn = get conn, Routes.admin_faq_path(conn, :show, faq)
      assert html_response(conn, 200) =~ "some updated answer"
    end

    test "renders errors when data is invalid", %{conn: conn, faq: faq} do
      conn = put conn, Routes.admin_faq_path(conn, :update, faq), faq: @invalid_attrs
      assert html_response(conn, 200) =~ "Edit Faq"
    end
  end
  ...
  describe "delete faq" do
    ...
    test "deletes chosen faq", %{conn: conn, faq: faq} do
      conn = delete conn, Routes.admin_faq_path(conn, :delete, faq)
